...
interface IFeeApprover {
    ...
    function setBoreTokenAddress(address _boreTokenAddress) external;
...
// File: contracts/IBoreVault.sol

pragma solidity ^0.6.0;

interface IBoreVault {
...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    function initialSetup(address router, address factory) internal {
        _name = "bnbVault.finance";
        _symbol = "BORE";
        _decimals = 18;
        _mint(address(this), initialSupply);
        contractStartTimestamp = block.timestamp;
        uniswapRouterV2 = IPancakeRouter02(
            router != address(0)
                ? router
                : 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F
        ); // For testing
        uniswapFactory = IPancakeFactory(
            factory != address(0)
                ? factory
    ...
    // which will put all the BNB and tokens inside the uniswap contract
    ...
    function addLiquidityToUniswapBORExWETHPair(address boreTeam) public onlyOwner{
        require(
        ...
        IPancakePair pair = IPancakePair(tokenUniswapPair);
        console.log("Balance of this", totalETHContributed / 1e18);
        //Wrap eth
        address WETH = uniswapRouterV2.WETH();
        // 20 % boreTeam claim
        uint256 twentyPercentClaim = totalETHContributed.mul(20).div(100);
        IWETH(WETH).deposit{value: totalETHContributed}();
        require(address(this).balance == 0, "Transfer Failed");
        IWETH(WETH).transfer(boreTeam, twentyPercentClaim);
        IWETH(WETH).transfer(address(pair), totalETHContributed.sub(twentyPercentClaim));
        ...
        LPperETHUnit = totalLPTokensMinted.mul(1e18).div(totalETHContributed.sub(twentyPercentClaim)); // 1e18x for  change
    ...
    function claimLPTokens() public {
        ...
        IPancakePair pair = IPancakePair(tokenUniswapPair);
        uint256 amountLPToTransfer =
            ethContributed[msg.sender].mul(LPperETHUnit).div(1e18);
    ...
    function _transfer(
        ...
        if (
            ...
            if (feeDistributor != address(0)) {
                IBoreVault(feeDistributor).addPendingRewards(
                    transferToFeeDistributorAmount
                );
            }
        }
...
// File: contracts/BORE.sol

pragma solidity 0.6.12;

// BoreToken with Governance.
contract BORE is NBUNIERC20 {
    /**
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "BORE::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "BORE::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "BORE::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BORE tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber =
            safe32(
                block.number,
                "BORE::_writeCheckpoint: block number exceeds 32 bits"
            );
