...
// File: contracts/BoreVault.sol

// Bore Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract BoreVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accBorePerShare) - user.rewardDebt
        //
    ...
    // Note contract owner is meant to be a governance contract allowing BORE governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's BOREs allocation point. Can only be called by the owner.
    // Note contract owner is meant to be a governance contract allowing BORE governance consensus
    ...
    function setDevFee(uint16 _DEV_FEE) public onlyOwner {
        require(_DEV_FEE <= 3000, "Dev fee clamped at 30%");
        DEV_FEE = _DEV_FEE;
    }
    ...
    function pendingBore(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        return user.amount.mul(accBorePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the BORE token.
    // ----
    uint256 private boreBalance;

    function addPendingRewards(uint256 _) public {
        uint256 newRewards = bore.balanceOf(address(this)).sub(boreBalance);

        if (newRewards > 0) {
            boreBalance = bore.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid)
        internal
        returns (uint256 boreRewardWhole)
    {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        boreRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 boreRewardFee = boreRewardWhole.mul(DEV_FEE).div(10000);
        uint256 boreRewardToDistribute = boreRewardWhole.sub(boreRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(boreRewardFee);

        pool.accBorePerShare = pool.accBorePerShare.add(
            boreRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    }

    // Deposit  tokens to BoreVault for BORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accBorePerShare).div(1e12);
    ...
    function depositFor(
        ...
        user.rewardDebt = user.amount.mul(pool.accBorePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from BoreVault.
    ...
    function _withdraw(
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid, from); // Update balances of from this is not withdrawal but claiming BORE farmed
    ...
    function updateAndPayOutPending(uint256 _pid, address from) internal {
        ...
        uint256 pending =
            user.amount.mul(pool.accBorePerShare).div(1e12).sub(
                user.rewardDebt
            );

        if (pending > 0) {
            safeBoreTransfer(from, pending);
        }
    }
    ...
    // Upcoming bYFI etc vaults strategy contracts will  se this function to manage and farm yield on value locked
    ...
    function safeBoreTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > boreBal) {
            ...
            boreBalance = bore.balanceOf(address(this));
        } else {
    ...
    function transferDevFee() public {
        ...
        uint256 boreBal = bore.balanceOf(address(this));
        if (pending_DEV_rewards > boreBal) {
            bore.transfer(devaddr, boreBal);
            boreBalance = bore.balanceOf(address(this));
        } else {
            bore.transfer(devaddr, pending_DEV_rewards);
            boreBalance = bore.balanceOf(address(this));
        }
    ...
    // allowing BORE governance token holders to do this functions.
