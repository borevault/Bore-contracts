...
// File: @pancakeswap/core/contracts/interfaces/IPancakeFactory.sol
...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        address _BOREAddress,
        address _WETHAddress,
        address _uniswapFactory
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        boreTokenAddress = _BOREAddress;
        WETHAddress = _WETHAddress;
        tokenUniswapPair = IPancakeFactory(_uniswapFactory).getPair(
            WETHAddress,
            boreTokenAddress
        );
        feePercentX100 = 22;
    ...
    function setBoreVaultAddress(address _boreVaultAddress) public onlyOwner {
        boreVaultAddress = _boreVaultAddress;
    }
        ...
        )
    {
        ...
        if (sender == boreVaultAddress || sender == tokenUniswapPair) {
            // Dont have a fee when borevault is sending, or infinite loop                       // And when pair is sending ( buys are happening, no tax on it)
